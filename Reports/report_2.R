...
plot(vgram(loc,y, lon.lat = TRUE, dmax = 15))
...
reml <- likfit(locc, ini = c(0.15,0.2), fix.nugget = T, method = "RML")
ols <- variofit(locc, ini = c(0.15,0.2), fix.nugget = T, weights="equal")
wls <- variofit(locc, ini = c(0.15,0.2), fix.nugget = T)
